...
class PluginManager:
    def __init__(self):
        self.plugins_module = importlib.import_module('papilio.plugins')
        self.plugin_names = [pluginname for _, pluginname, ispkg in pkgutil.walk_packages(self.plugins_module.__path__) if ispkg]
        self.plugins = [importlib.import_module('papilio.plugins.'+plugin_name) for plugin_name in self.plugin_names]
