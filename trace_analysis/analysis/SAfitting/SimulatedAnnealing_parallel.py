...
#import cas9_model.CRISPR_dCas9_binding_curve_Boyle as dCas9Boyle
...
def sim_anneal_fit(xdata, ydata, yerr, Xstart, lwrbnd, upbnd, model='I_am_using_multi_processing_in_stead',
                ...
                Tstart=0.1, delta=0.05, delta2=2.0, tol=1E-3, Tfinal=0.01, adjust_factor=1.1, cooling_rate=0.85, N_int=1000,
...
class SimAnneal():
    ...
    delta & delta2 : initial step size for parameter changes
...
def TakeStep(SA, X, lwrbnd, upbnd):
    ...
    '''
    delta = SA.step_size
    delta2 = SA.step_size2
    Xtrial = np.zeros(len(X))

    if SA.RelativeSteps:
        X = np.log(X)
        Xtrial[0] = np.random.uniform(np.max([X[0] - delta, np.log(lwrbnd[0])]),
                                          np.min([X[0] + delta, np.log(upbnd[0])]))
        for i in range(1, len(X)):
            Xtrial[i] = np.random.uniform(np.max([X[i] - delta2, np.log(lwrbnd[i])]),
                                          np.min([X[i] + delta2, np.log(upbnd[i])]))
    else:
        Xtrial[0] = np.random.uniform(np.max([X[0] - delta, lwrbnd[0]]),
                                          np.min([X[0] + delta, upbnd[0]]))
        for i in range(1, len(X)):
            Xtrial[i] = np.random.uniform(np.max([X[i] - delta2, lwrbnd[i]]),
                                          np.min([X[i] + delta2, upbnd[i]]))
...
def Temperature_Cycle(SA, Eavg):
    ...
    Ediff = abs(SA.average_energy - Eavg)/SA.average_energy
    print('Ediff ', Ediff)
    tolerance_low_enough = Ediff < SA.Tolerance
