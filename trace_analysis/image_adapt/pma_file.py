...
#        self.m_offset = self.filesize - self.datasize - 8
    ...
    def read_frame(self, frame_number,ii=0):
        with self.filepath.open('rb') as fid:
            np.fromfile(fid, np.int16,count=1)
            np.fromfile(fid, np.int16,count=1)
    ...
    #        with open(root+'\\'+name, 'rb') as fid: #did the offset reset?    # is already open
                ...
                im = np.reshape(np.fromfile(fid,np.int8,count=self.width*self.height),(self.width,self.height))
            else:
    #        with open(root+'\\'+name, 'rb') as fid: #did the offset reset?  #is already open
                fid.seek(4+ 2*frame_number*(self.width*self.height), os.SEEK_SET)
                msb=np.reshape(np.fromfile(fid,np.int8,count=(self.width*self.height)),(self.width,self.height))
                lsb=np.reshape(np.fromfile(fid,np.int8,count=(self.width*self.height)),(self.width,self.height))
