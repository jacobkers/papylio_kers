# -*- coding: utf-8 -*-
"""
Created on Wed May  8 01:50:26 2019
...
class SnaptoCursor(object):
    ...
    def __init__(self, ax, x, y):
        self.ax = ax
        self.lx = ax.axhline(color='white', ls="--", lw=1, alpha=0.5)  # the horiz line
        self.ly = ax.axvline(color='white', ls="--", lw=1, alpha=0.5)  # the vert line
        self.lx.set_visible(False)
        self.ly.set_visible(False)
        self.x = x
        self.y = y
        # text location in axes coords
        self.txt = ax.text(0.78, 1.06, '', transform=ax.transAxes)
    def mouse_move(self, event):
        ...
        try:
            ...
            self.leave_axis(event)
        # update the line positions
        self.lx.set_ydata(y)
        self.ly.set_xdata(x)
        self.lx.set_visible(True)
        self.ly.set_visible(True)
#        self.txt.set_text(f't = {x:.1f} s' )
#
